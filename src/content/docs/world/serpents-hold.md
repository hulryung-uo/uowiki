---
title: Serpent's Hold
description: The island fortress of the Order of the Silver Serpent — a warrior's bastion in the southern sea.
status: source-verified
sources:
  - "anima: data/map_pois.json (city + npc_vendor: Serpent's Hold)"
  - "anima: data/world_knowledge.yaml (cities.serpents_hold)"
  - "servuo: Data/Regions.xml (TownRegion \"Serpent's Hold\", go 3010,3371, rect 2868,3324 205x195; present on both Felucca and Trammel facets)"
  - "servuo: Scripts/Regions/TownRegion.cs (TownRegion : GuardedRegion — guarded by default)"
  - "servuo: Scripts/Items/Functional/PublicMoongate.cs (no Serpent's Hold public-gate entry)"
  - "reference: uo.com cities & towns"
last_verified: 2026-06-22
generated: false
---

Serpent's Hold is a fortress before it is a town — a stout keep on a southern island, garrisoned
by the **Order of the Silver Serpent** and dedicated to martial discipline. Where Jhelom is a
rowdy fighter's island, Serpent's Hold is the disciplined one: a place of training halls,
weapon racks, and knights who take their duty seriously. The walls keep the southern seas at
bay, and within them a surprisingly complete set of trades supports the garrison. Adventurers
come for the martial atmosphere, the safety of the keep, and the launch point it offers into
the wild southern waters.

[Open Serpent's Hold in the interactive map →](https://uomap.vercel.app/?facet=felucca&x=2895&y=3480&z=2)

## Getting there

- **Center:** (2895, 3480) — an island fortress in the **southern** sea, south-east of
  [Trinsic](/world/trinsic/). Verified region travel point is (3010, 3371). It is a
  **guarded town** (`Data/Regions.xml`, `TownRegion`/`GuardedRegion`) and exists on both the
  Felucca and Trammel facets.
- There is **no public moongate** to Serpent's Hold; reach it by **boat**, or by
  [*Recall*](/skills/magery/) / *Gate Travel* to a marked rune. See
  [Moongates & shrines](/world/moongates-and-shrines/) for the nearest mainland gates.

## Services

A complete garrison-town roster, from the NPC vendors recorded within the keep
([what the icons mean](/world/town-services/)):

<img src="/img/services/bank.gif" alt="Bank" width="28" /> Bank ·
<img src="/img/services/blacksmith.gif" alt="Blacksmith" width="28" /> Blacksmith ·
<img src="/img/services/tinker.gif" alt="Tinker" width="28" /> Tinker ·
<img src="/img/services/bowyer.gif" alt="Bowyer" width="28" /> Bowyer ·
<img src="/img/services/tailor.gif" alt="Tailor" width="28" /> Tailor ·
<img src="/img/services/mage.gif" alt="Mage shop" width="28" /> Mage shop ·
<img src="/img/services/reagents.gif" alt="Reagents" width="28" /> Reagents ·
<img src="/img/services/healer.gif" alt="Healer" width="28" /> Healer ·
<img src="/img/services/provisioner.gif" alt="Provisioner" width="28" /> Provisioner ·
<img src="/img/services/baker.gif" alt="Baker" width="28" /> Baker ·
<img src="/img/services/stables.gif" alt="Stables" width="28" /> Stables ·
<img src="/img/services/inn.gif" alt="Inn" width="28" /> Inn ·
<img src="/img/services/tavern.gif" alt="Tavern" width="28" /> Tavern

The smith is an **armorer-and-weaponsmith** befitting a knightly order, with a healer and inn
close by. A fisherman works the docks.

## Nearby

- **The southern sea** surrounds the fortress — open water and [fishing](/professions/fisher/)
  in every direction.
- The keep itself, home to the Order of the Silver Serpent, is the island's defining landmark.

## Related

- [Trinsic](/world/trinsic/) · [Jhelom](/world/jhelom/) — the other martial cities of the south
- [Warrior](/professions/warrior/) · [Paladin](/professions/paladin/)
- [Town services legend](/world/town-services/) · [Vendors & banking](/playing/vendors-and-banking/)
- [World overview](/world/) · [Moongates and shrines](/world/moongates-and-shrines/)
